...
<template>
  <div class="min-h-screen">
    ...
    <router-link
      ...
      to="/support"
      class="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-primary-500 text-2xl shadow-lg shadow-primary-500/30 transition-transform hover:scale-110"
      title="AI 客服"
    >
      🤖
    </router-link>
    ...
    <footer v-if="!hideFooter" class="border-t border-white/5 bg-dark-base/70">
      <div class="shell-container flex flex-col gap-4 py-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <router-link v-if="isAuthenticated && !isAdmin" to="/services" class="filter-pill">{{ isBooster ? copy.boosterDesk : copy.serviceMarket }}</router-link>
          <router-link v-if="isAuthenticated" to="/support" class="filter-pill">AI客服</router-link>
        </div>
      </div>
    </footer>
  </div>
