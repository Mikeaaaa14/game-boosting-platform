  ...
  <div class="page-shell">
    <div class="mx-auto flex h-[calc(100vh-120px)] max-w-4xl flex-col">
      ...
      <div class="flex items-center gap-3 border-b border-white/10 pb-4">
        <div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary-500/20 text-lg">
          🤖
        </div>
        <div>
      ...
      <div ref="chatContainer" class="flex-1 space-y-4 overflow-y-auto py-4">
        ...
        <div class="flex gap-3">
          <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-primary-500/20 text-sm">🤖</div>
          <div class="max-w-[80%] rounded-2xl rounded-tl-sm border border-white/10 bg-white/5 p-4">
            <p class="whitespace-pre-line text-sm text-slate-200">{{ greeting }}</p>
          </div>
        </div>

        <!-- Template quick replies -->
        <div v-if="messages.length === 0" class="space-y-3 pl-11">
        ...
        <template v-for="msg in messages" :key="msg.id">
          ...
          <div v-if="msg.role === 'user'" class="flex justify-end gap-3">
            <div class="max-w-[80%] rounded-2xl rounded-tr-sm bg-primary-500/20 p-4">
              <p class="text-sm text-white">{{ msg.text }}</p>
              <p class="mt-1 text-right text-[10px] text-slate-500">{{ msg.time }}</p>
            </div>
          </div>
          ...
          <div v-else class="flex gap-3">
            ...
            <div class="max-w-[80%] space-y-3">
              <div class="rounded-2xl rounded-tl-sm border border-white/10 bg-white/5 p-4">
              ...
              <div v-if="msg.actions?.length" class="flex flex-wrap gap-2">
                <button
                  ...
                  :key="idx"
                  class="btn-secondary !rounded-xl !px-3 !py-1.5 text-xs"
              ...
              <div v-if="msg.needHuman && !msg.actions?.some(a => a.type === 'transfer')" class="flex gap-2">
                <button class="btn-ghost !rounded-xl !px-3 !py-1.5 text-xs text-amber-300" @click="transferToHuman">
                  建议转人工客服
                </button>
              </div>
            </div>
        ...
        <div v-if="sending" class="flex gap-3">
          <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-primary-500/20 text-sm">🤖</div>
          <div class="rounded-2xl rounded-tl-sm border border-white/10 bg-white/5 px-4 py-3">
      ...
      <div v-if="messages.length > 0" class="border-t border-white/10 py-2">
      ...
      <!-- Input area -->
      <div class="border-t border-white/10 pt-3 pb-2">
