    ...
    <div class="mx-auto max-w-3xl space-y-6">
          ...
          >
            ...
            <label class="relative inline-flex cursor-pointer items-center">
              <input
                ...
                :false-value="false"
              />
              <div class="h-6 w-11 rounded-full bg-slate-600 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all after:content-[''] peer-checked:bg-primary-500 peer-checked:after:translate-x-full peer-focus:outline-none"></div>
            </label>
          </div>
        </div>
      ...
      <section v-if="activeTab === 'cache'" class="surface-card space-y-5 p-6">
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4">
            <div>
              <p class="text-sm font-medium text-amber-100">清除所有本地数据</p>
              <p class="mt-0.5 text-xs text-amber-200/60">重置所有本地缓存（保留登录状态）</p>
            </div>
            <button class="btn-ghost !px-4 text-sm text-amber-300" @click="clearAllLocalData">清除全部</button>
          </div>
        </div>
      </section>
    </div>
