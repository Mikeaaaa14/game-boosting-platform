    ...
    <section class="hero-panel scanline-overlay p-6 sm:p-8">
      <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex items-center gap-5">
          <div class="flex h-20 w-20 items-center justify-center rounded-[28px] border border-primary-300/45 bg-primary-500/10 text-3xl font-semibold text-primary-100">
            {{ avatarText }}
          </div>
          <div class="space-y-2">
        ...
        <div class="grid gap-4" :class="user?.role === 'BOOSTER' ? 'sm:grid-cols-4' : 'sm:grid-cols-3'">
          ...
          <article v-if="user?.role === 'BOOSTER' && application" class="stat-card"><p class="text-sm text-slate-400">接单额度</p><p class="mt-2 text-lg font-semibold text-white">{{ application.booster_quota ?? 0 }} 单</p></article>
        </div>
      </div>
    </section>
      ...
      <aside class="surface-card p-6 sm:p-8">
        ...
        <div class="mt-6 grid gap-4">
          <div class="rounded-3xl border border-white/10 bg-white/5 p-4">
            ...
            <p class="mt-2 text-sm font-medium text-white">{{ applicationMeta.description }}</p>
          </div>
          <div v-if="application?.review_note" class="rounded-3xl border border-amber-400/20 bg-amber-400/10 p-4">
            <p class="text-xs uppercase tracking-[0.18em] text-amber-100">备注</p>
        ...
        <form v-if="shouldShowApplicationForm" class="mt-6 space-y-5" @submit.prevent="submitBoosterApplication">
          ...
          <div class="rounded-3xl border border-dashed border-primary-300/40 bg-white/5 p-4">
        ...
        <div v-else class="mt-6 rounded-3xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
