...
const canOrder = computed(() => authStore.isAuthenticated && !authStore.isBooster && !isOwner.value)
...
<template>
  <div class="page-shell space-y-8">
        ...
        <section class="surface-card p-6 sm:p-8">
          ...
          <div class="mt-6 grid gap-4 sm:grid-cols-2">
            <div class="rounded-3xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.18em] text-slate-500">服务编号</p>
              <p class="mt-2 text-sm font-medium text-white">#{{ service.id }}</p>
            </div>
            <div class="rounded-3xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.18em] text-slate-500">所属游戏</p>
              <p class="mt-2 text-sm font-medium text-white">{{ game?.name || `游戏 #${service.game_id}` }}</p>
            </div>
            <div class="rounded-3xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.18em] text-slate-500">发布者</p>
              <p class="mt-2 text-sm font-medium text-white">代练 #{{ service.booster_id }}</p>
            </div>
            <div class="rounded-3xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.18em] text-slate-500">平台</p>
              <p class="mt-2 text-sm font-medium text-white">{{ getGamePlatformLabel(game?.platform) }}</p>
            </div>
          </div>
    ...
    <section v-else class="empty-panel">
      <h2 class="text-2xl font-semibold text-white">服务不存在或暂时不可见</h2>
      <p class="mt-3 text-sm leading-7 text-slate-400">
        这张服务卡片可能已被下架，或者你暂时没有权限查看。
      </p>
    </section>
  </div>
</template>
