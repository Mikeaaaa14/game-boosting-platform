...
<template>
    ...
    >
      ...
      >
        ...
        <button
          v-if="isImage"
          type="button"
          class="block overflow-hidden rounded-[20px] border border-white/10 bg-dark-base/50"
          @click="openPreview"
        >
          <img
            :src="imageUrl"
  ...
  <teleport to="body">
    ...
    >
      <button
        type="button"
        class="absolute right-6 top-6 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white transition hover:border-primary-300/40 hover:text-primary-100"
        @click="closePreview"
      >
        关闭
      </button>
      <img
        :src="imageUrl"
        alt="聊天图片预览"
        class="max-h-full max-w-full rounded-[24px] border border-white/10 object-contain shadow-[0_0_40px_rgba(0,240,255,0.15)]"
      />
    </div>
  </teleport>
</template>
